...
export default function HotelsPage() {
  ...
  useEffect(() => {
    fetch("/api/properties?type=Hotel")
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      <Head>
        <title>Hotels - Jconnect Zambia</title>
        <meta
          name="description"
          content="Browse hotels in Zambia. Search, filter, and book your next stay."
        />
      </Head>
      <div
      ...
      >
        ...
        <h1 className="text-4xl font-extrabold mb-6 text-yellow-900 text-center">
          Hotels
        </h1>
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
              {filtered
                ...
                .map((property) => (
                  <Link href={`/hotels/${property.id}`} key={property.id}>
      ...
      <p className="mb-8 text-xl text-center text-gray-700">
        Browse hotels in Zambia. Search, filter, and book your next stay.
      </p>
    </>
  );
}
